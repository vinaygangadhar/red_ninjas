__global__ void transpose_kernel(int* sum, int *trans_sum, int* sqsum, int *trans_sqsum, int w, int h)
{
     ...
     volatile __shared__ int sum_temp[16][17];
     volatile __shared__ int sqsum_temp[16][17];
     ...
     if (row < h && col < w)
     {
          trans_sum[col * h + row] = sum_temp[tx][ty];
          trans_sqsum[col * h + row] = sqsum_temp[tx][ty];
     }
