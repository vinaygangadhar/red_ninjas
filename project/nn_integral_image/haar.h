...
typedef  int16_t sumtype16_t;
typedef int16_t sqsumtype16_t;
...
//    MySize real_window_size;
    ...
    MyIntImage16_t sum;
    MyIntImage16_t sqsum;
   
    // pointers to the corner of the actual detection window
    sqsumtype16_t *pq0, *pq1, *pq2, *pq3;
    sumtype16_t *p0, *p1, *p2, *p3;
...
void setImageForCascadeClassifier16_t( myCascade16_t* cascade, MyIntImage16_t* sum, MyIntImage16_t* sqsum);

/* runs the cascade on the specified window */
int runCascadeClassifier( myCascade* cascade, MyPoint pt, int start_stage);
int runCascadeClassifier16_t( myCascade16_t* cascade, MyPoint pt, int start_stage);
...
std::vector<MyRect> detectObjects16_t( MyImage* image, MySize minSize, MySize maxSize,
