...
static int16_t **scaled_rectangles_array16_t;
...
void integralImageOnHost16_t( MyImage *src, MyIntImage16_t *sum, MyIntImage16_t *sqsum );
...
void ScaleImage_Invoker16_t( myCascade16_t* _cascade, float _factor, int16_t sum_row, int16_t sum_col, std::vector<MyRect>& _vec);
   ...
   *****************************************************/
   ...
   for( factor = 1; ; factor *= scaleFactor )
   {
      ...
      ***************************************************/
      ...
      nn_integralImageOnDevice(img, devicesum1, devicesqsum1);
...
//16-bit version
std::vector<MyRect> detectObjects16_t( MyImage* _img, MySize minSize, MySize maxSize, myCascade16_t* cascade,
    float scaleFactor, int minNeighbors, std::fstream& ofs)
{
   ...
   **********************************/
   ...
   MyImage image1Obj;
   MyIntImage16_t sum1Obj;
   MyIntImage16_t sqsum1Obj;

   MyIntImage16_t sumDevice1Obj;
   MyIntImage16_t sqsumDevice1Obj;

   /* pointers for the created structs */
   MyImage *img1 = &image1Obj;
   MyIntImage16_t *sum1 = &sum1Obj;
   MyIntImage16_t *sqsum1 = &sqsum1Obj;
   
   MyIntImage16_t *devicesum1 = &sumDevice1Obj;
   MyIntImage16_t *devicesqsum1 = &sqsumDevice1Obj;
   ...
   *****************************************************/
   ...
   /* malloc for sum1: unsigned char */
   createSumImage16_t(img->width, img->height, sum1);
   createSumImage16_t(img->width, img->height, devicesum1);
   /* malloc for sqsum1: unsigned char */
   createSumImage16_t(img->width, img->height, sqsum1);
   createSumImage16_t(img->width, img->height, devicesqsum1);
   ...
   for( factor = 1; ; factor *= scaleFactor )
   {
      ...
      ************************************/
     setImage(sz.width, sz.height, img1);
     setSumImage16_t(sz.width, sz.height, sum1);
     setSumImage16_t(sz.width, sz.height, sqsum1);
     
     setSumImage16_t(sz.width, sz.height, devicesum1);
     setSumImage16_t(sz.width, sz.height, devicesqsum1);
      ...
      **************************************/
      ...
      //NN's downsampled image is passed to II 
      nearestNeighborOnHost(img, img1);
      integralImageOnHost16_t(img1, sum1, sqsum1);
      ...
      ***************************************************/
      ...
      nn_integralImageOnDevice16_t(img, devicesum1, devicesqsum1);

      if(PRINT_LOG){ 
         //Compare the host and device results
         
         if( !CompareResultsInt16_t(sum1->data, devicesum1->data, sqsum1->data, devicesqsum1->data, img1->width * img1->height) )
         {
      ...
      * The computation is done next in ScaleImage_Invoker
      *************************************************/
     setImageForCascadeClassifier16_t( cascade, sum1, sqsum1);
      ...
      * the same cascade filter is invoked each time
      ***************************************************/
     ScaleImage_Invoker16_t(cascade, factor, sum1->height, sum1->width,
         allCandidates);
   ...
   freeSumImage16_t(sum1);
   freeSumImage16_t(sqsum1);
...
//16bit version
void setImageForCascadeClassifier16_t( myCascade16_t* _cascade, MyIntImage16_t* _sum, MyIntImage16_t* _sqsum)
{
  MyIntImage16_t *sum = _sum;
  MyIntImage16_t *sqsum = _sqsum;
  myCascade16_t *cascade = _cascade;
  ...
  for( i = 0; i < cascade->n_stages; i++ )
  {
    ...
    for( j = 0; j < stages_array[i]; j++ )
    {
      ...
      for( k = 0; k < nr; k++ )
      {
        ...
        if (k < 2)
        {
          scaled_rectangles_array16_t[r_index + k*4] =     (sum->data + sum->width*(tr.y ) + (tr.x )) ;
          scaled_rectangles_array16_t[r_index + k*4 + 1] = (sum->data + sum->width*(tr.y ) + (tr.x  + tr.width)) ;
          scaled_rectangles_array16_t[r_index + k*4 + 2] = (sum->data + sum->width*(tr.y  + tr.height) + (tr.x ));
          scaled_rectangles_array16_t[r_index + k*4 + 3] = (sum->data + sum->width*(tr.y  + tr.height) + (tr.x  + tr.width));
        }
        else
        {
          if ((tr.x == 0)&& (tr.y == 0) &&(tr.width == 0) &&(tr.height == 0))
          {
            scaled_rectangles_array16_t[r_index + k*4] = NULL ;
            scaled_rectangles_array16_t[r_index + k*4 + 1] = NULL ;
            scaled_rectangles_array16_t[r_index + k*4 + 2] = NULL;
            scaled_rectangles_array16_t[r_index + k*4 + 3] = NULL;
          }
          else
          {
            scaled_rectangles_array16_t[r_index + k*4] =     (sum->data + sum->width*(tr.y ) + (tr.x )) ;
...
//16bit verrsion
inline int evalWeakClassifier16_t(int variance_norm_factor, int p_offset, int tree_index, int w_index, int r_index )
{

  /* the node threshold is multiplied by the standard deviation of the image */
  int t = tree_thresh_array[tree_index] * variance_norm_factor;

  int sum = (*(scaled_rectangles_array16_t[r_index] + p_offset)
      - *(scaled_rectangles_array16_t[r_index + 1] + p_offset)
      - *(scaled_rectangles_array16_t[r_index + 2] + p_offset)
      + *(scaled_rectangles_array16_t[r_index + 3] + p_offset))
    * weights_array[w_index];


  sum += (*(scaled_rectangles_array16_t[r_index+4] + p_offset)
      - *(scaled_rectangles_array16_t[r_index + 5] + p_offset)
      - *(scaled_rectangles_array16_t[r_index + 6] + p_offset)
      + *(scaled_rectangles_array16_t[r_index + 7] + p_offset))
    * weights_array[w_index + 1];

  if ((scaled_rectangles_array16_t[r_index+8] != NULL))
    sum += (*(scaled_rectangles_array16_t[r_index+8] + p_offset)
        - *(scaled_rectangles_array16_t[r_index + 9] + p_offset)
        - *(scaled_rectangles_array16_t[r_index + 10] + p_offset)
        + *(scaled_rectangles_array16_t[r_index + 11] + p_offset))
      * weights_array[w_index + 2];
...
//32bit version
int runCascadeClassifier16_t( myCascade16_t* _cascade, MyPoint pt, int start_stage )
{
  ...
  for( i = start_stage; i < cascade->n_stages; i++ )
  {
    ...
    for( j = 0; j < stages_array[i]; j++ )
    {
       ...
       * Send the shifted window to a haar filter.
       **************************************************/
      stage_sum += evalWeakClassifier16_t(variance_norm_factor, p_offset, haar_counter, w_index, r_index);
      n_features++;
      haar_counter++;
      w_index+=3;
      r_index+=12;
...
void ScaleImage_Invoker16_t( myCascade16_t* _cascade, float _factor, int16_t sum_row, int16_t sum_col, std::vector<MyRect>& _vec)
{
  ...
  for( x = 0; x <= x2; x += step )
    for( y = y1; y <= y2; y += step )
    {
       ...
       * The same cascade filter is used each time
       ********************************************/
      result = runCascadeClassifier16_t( cascade, p, 0 );
...
void integralImageOnHost16_t( MyImage *src, MyIntImage16_t *sum, MyIntImage16_t *sqsum )
{
    ...
    int16_t *sumData = sum->data;
    int16_t *sqsumData = sqsum->data;
...
void readTextClassifier()//(myCascade * cascade)
{
  ...
  * (in the 5kk73 example, there are 25 stages)
  ...
  scaled_rectangles_array16_t = (int16_t **)malloc(sizeof(int16_t*)*total_nodes*12);
...
void releaseTextClassifier()
{
  ...
  free(scaled_rectangles_array16_t);
