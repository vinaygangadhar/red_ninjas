...
//void nn_integralImageOnDevice(MyImage *src, MyImage *dst, MyIntDeviceImage *sum, MyIntDeviceImage *sqsum )
...
//                                                                        threadsPerBlock_rs);
     ...
     //Column Scan
     column_scan_kernel<<<blocksPerGrid_cs, (threadsPerBlock_cs / 2)>>>(d_sum.data, d_sqsum.data, dst_h, threadsPerBlock_cs);
     
     check = CUDA_CHECK_RETURN( cudaPeekAtLastError(), __FILE__, __LINE__ );
     if( check != 0){
           std::cerr << "Error: CudaPeek on Column Scan not successfull for device dest image" << std::endl;
           exit(1);
     }
     ...
     printf("\tII: Columnscan Done on GPU-%d: Exclusive Time: %f ms\n", __LINE__, exc_msecTotal);
